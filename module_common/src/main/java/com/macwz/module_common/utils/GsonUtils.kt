package com.macwz.module_common.utils
...
object GsonUtils {
    ...
    public fun getNoteJsonString(jsonString: String, note: String): String {
    ...
    fun <T> parserJsonToArrayBeans(jsonString: String, beanClazz: Class<T>): List<T> {
        ...
        for (jsonElement2 in jsonArray) {
            val bean = mGson.fromJson(jsonElement2, beanClazz)
            beans.add(bean)
        }
        return beans
    }
    ...
    fun <T> parserJsonToBean(jsonString: String, clazzBean: Class<T>): T {
        ...
        return mGson.fromJson(jsonElement, clazzBean)
    }
